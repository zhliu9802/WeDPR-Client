...
struct SymDecData {
    ...
    InputBuffer iv;
    InputBuffer symSk;
    char* symDecryptResult = NULL;
};


// eg:sym_dec(columnName, algorithmType, mode, hexIv, hexSk)
my_bool sym_dec_init(UDF_INIT* initid, UDF_ARGS* args, char* message)
{
    SymDecData* data = new SymDecData;
    ...
    {
        snprintf(message, max_msg_size, "%s", "sym_dec() requires (string, int, int, string, string) as parameters");
        return 1;
    }
    ...
    data->symDecryptResult = (char*)malloc(symmetric_block_size(data->algorithmType) + args->lengths[0] / 2);
    if (!data->symDecryptResult)
    {
        snprintf(message, max_msg_size, "%s", "Malloc memory for symDecryptResult failed!");
        return 1;
    }
...
void sym_dec_deinit(UDF_INIT* initid)
{
    if (initid->ptr)
    {
        SymDecData* data = (SymDecData*)initid->ptr;
        if(data->symDecryptResult)
        {
            free(data->symDecryptResult);
            data->symDecryptResult = NULL;
        }
        if(data->iv.data)
        {
...
char* sym_dec(UDF_INIT* initid, UDF_ARGS* args, char* result, unsigned long* length, char* is_null,
    char* error)
{
    ...
    OutputBuffer plainBuffer = {(unsigned char*)(data->symDecryptResult),
        symmetric_block_size(data->algorithmType) + args->lengths[0] / 2};
    symmetric_decrypt(&plainBuffer, data->algorithmType, data->mode, &data->symSk, &data->iv, &ciphertext);
    if (get_last_error() != 0)
    {

 ...
 *
 * @reference https://github.com/microsoft/APSI/sender/bin_bundle.h
 * @license MIT license
...
struct BatchedPlaintextPolyn
{
    ...
    */
    BatchedPlaintextPolyn(const std::vector<FEltPolyn>& polyns, apsi::CryptoContext context,
        std::uint32_t ps_low_degree, bool compressed);

    /**
    Constructs an uninitialized Plaintext polynomial using the given crypto context
    */
    BatchedPlaintextPolyn(apsi::CryptoContext context) : crypto_context(std::move(context)) {}
    ...
    */
    seal::Ciphertext eval_patstock(const apsi::CryptoContext& eval_crypto_context,
        const std::vector<seal::Ciphertext>& ciphertext_powers, std::size_t ps_low_degree,
        seal::MemoryPoolHandle& pool) const;

    /**
    Returns whether this polynomial has non-zero size.
    */
    explicit operator bool() const noexcept { return batched_coeffs.size(); }
};
...
struct BinBundleCache
{
    ...
    BinBundleCache(const apsi::CryptoContext& crypto_context, std::size_t label_size);
...
private:
    ...
    */
    std::vector<apsi::sender::util::CuckooFilter> filters_;
...
public:
    BinBundle(const apsi::CryptoContext& crypto_context, std::size_t label_size,
        std::size_t max_bin_size, std::size_t ps_low_degree, std::size_t num_bins, bool compressed,
        bool stripped);
    ...
    */
    bool try_get_multi_label(const std::vector<felt_t>& items, std::size_t start_bin_idx,
    ...
    */
    bool cache_invalid() const noexcept { return cache_invalid_; }
    ...
    */
    const std::vector<std::vector<felt_t>>& get_item_bins() const noexcept { return item_bins_; }

    /**
    Returns the size of the label in multiples of the item size.
    */
    std::size_t get_label_size() const noexcept { return label_size_; }

    /**
    Returns the number of bins.
    */
    std::size_t get_num_bins() const noexcept { return num_bins_; }
    ...
    */
    ...
    ppctars::BinBundle saveToTarsBinBundle() const;

    static BinBundle loadFromTarsBinBundle(
        const apsi::CryptoContext& _cryptoContext, const ppctars::BinBundle& _tarsBinBundle);
};

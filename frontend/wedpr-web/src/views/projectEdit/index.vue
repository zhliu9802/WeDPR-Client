<template>
  <div class="create-data">
    ...
    <div>
      <el-button size="medium" icon="el-icon-edit" type="primary" @click="submit"> 确认修改 </el-button>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async updateProject(params) {
      const res = await projectManageServer.updateProject(params)
      console.log(res)
      if (res.code === 0) {
    ...
    submit() {
      this.$refs.dataForm.validate((valid) => {
        if (valid) {
          ...
          const { projectId } = this
          this.updateProject({ project: { name, projectDesc, type, id: projectId } })
        }
      })
    }
  }
